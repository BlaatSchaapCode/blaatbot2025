...
void TcpConnection::send(std::vector<char> data) {
  size_t sent_bytes =
      ::send(m_socket, (const char *)data.data(), (int)data.size(), 0);
...
void TcpConnection::receiveThreadFunc(TcpConnection *self) {
  ...
  while (self->m_receiveThreadActive) {
    ...
    if (bytes_received < 0) {
      ...
      std::vector<char> received_data(recv_buffer,
...
int TcpConnection::connect(void) {
  ...
  while (res && !m_connected) {
    ...
    case AF_INET: {
      ...
      LOG_INFO("%s resolved to %s", mHostName.c_str(),
               inet_ntop(AF_INET, &in.sin_addr, temp, INET_ADDRSTRLEN));
    ...
    case AF_INET6: {
      ...
      LOG_INFO("%s resolved to %s", mHostName.c_str(),
               inet_ntop(AF_INET6, &in6.sin6_addr, temp, INET6_ADDRSTRLEN));
      ...
      if (::connect(m_socket, (const sockaddr *)&in6, sizeof(sockaddr_in6)) <
          0) {
        LOG_INFO("Failed to connect");
        close(m_socket);
        m_socket = 0;
  ...
  setsockopt(m_socket, SOL_SOCKET, SO_KEEPALIVE, (const char *)&yes,
             sizeof(yes));
...
void deleteInstance(network::TcpConnection *inst) { delete inst; }
}
#endif
