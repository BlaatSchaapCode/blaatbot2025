...
IRC::IRC() {
  ...
  mMessageParsers["PONG"] = [this](const IRCMessage message) {
    onPONG(message);
  };
  mMessageParsers["PING"] = [this](const IRCMessage message) {
    onPING(message);
  };
  mMessageParsers["PRIVMSG"] = [this](const IRCMessage message) {
    onPRIVMSG(message);
  };
  mMessageParsers["NOTICE"] = [this](const IRCMessage message) {
    onNOTICE(message);
  };
  mMessageParsers["ERROR"] = [this](const IRCMessage message) {
    onERROR(message);
  };
  mMessageParsers[Numeric::ERR_NICKNAMEINUSE] =
      [this](const IRCMessage message) { onNicknameInUse(message); };

  mMessageParsers[Numeric::RPL_ENDOFMOTD] = [this](const IRCMessage message) {
    onReady();
  };
  mMessageParsers[Numeric::ERR_NOMOTD] = [this](const IRCMessage message) {
    onReady();
  };

  mMessageParsers["CAP"] = [this](const IRCMessage message) { onCAP(message); };
  mMessageParsers[Numeric::IRCRPL_IRCX] = [this](const IRCMessage message) {
    onIRCX(message);
  };

  mMessageParsers[Numeric::ERR_UNKNOWNCOMMAND] =
      [this](const IRCMessage message) { onUnknownCommand(message); };

  mMessageParsers[Numeric::RPL_CREATED] = [this](const IRCMessage message) {
    onCreated(message);
  };
  mMessageParsers[Numeric::RPL_MYINFO] = [this](const IRCMessage message) {
    onMyInfo(message);
  };
  mMessageParsers[Numeric::RPL_ISUPPORT] = [this](const IRCMessage message) {
    onISupport(message);
  };

  mMessageParsers[Numeric::RPL_WELCOME] = [this](const IRCMessage message) {
    onWelcome(message);
  };
  mMessageParsers[Numeric::RPL_YOURHOST] = [this](const IRCMessage message) {
    onYourHost(message);
  };

  mMessageParsers["JOIN"] = [this](const IRCMessage message) {
    onJOIN(message);
  };
  mMessageParsers["MODE"] = [this](const IRCMessage message) {
    onMODE(message);
  };
  mMessageParsers[Numeric::RPL_CHANNELMODEIS] =
      [this](const IRCMessage message) { onChannelModeIs(message); };

  mMessageParsers[Numeric::RPL_TOPIC] = [this](const IRCMessage message) {
    onTopic(message);
  };
  mMessageParsers[Numeric::RPL_TOPICWHOTIME] =
      [this](const IRCMessage message) { onTopicWhoTime(message); };
  mMessageParsers[Numeric::RPL_NAMREPLY] = [this](const IRCMessage message) {
    onNamReply(message);
  };
  mMessageParsers[Numeric::RPL_ENDOFNAMES] = [this](const IRCMessage message) {
    onEndOfNames(message);
  };

  mMessageParsers[Numeric::RPL_WHOREPLY] = [this](const IRCMessage message) {
    onWhoReply(message);
  };
  mMessageParsers[Numeric::RPL_WHOSPCRPL] = [this](const IRCMessage message) {
    onWhoSpcReply(message);
  };
  mMessageParsers[Numeric::RPL_ENDOFWHO] = [this](const IRCMessage message) {
    onEndOfWho(message);
  };
  //
  //
...
std::string IRC::toLower(std::string s) {
  ...
  if ((caseMapping == "rfc3454") || (caseMapping == "rfc7700") ||
      (caseMapping == "rfc7613") || (caseMapping == "rfc8265") ||
...
void IRC::onConnected() {
  ...
  connectTimer.afterSeconds([this]() { onCanRegister(); },
...
void IRC::onWelcome(const IRCMessage message) {
  ...
  if (message.parameters.size() > 0) {
    ...
    if (preNetPos != std::string::npos && postNetPos != std::string::npos) {
      serverInfo.network = message.parameters[1].substr(
          preNetString.length() + preNetPos,
          postNetPos - preNetPos - preNetString.length());
      LOG_INFO("Network : %s", serverInfo.network.c_str());
      std::string nickUserHost =
          message.parameters[1].substr(postNetString.length() + postNetPos + 1);
...
void IRC::onISupport(const IRCMessage message) {
  ...
  if (message.parameters.size() > 2) {
    if (message.parameters[message.parameters.size() - 1] == iSupport) {
      ...
      isupprt.insert(isupprt.end(), message.parameters.begin() + 1,
                     message.parameters.end() - 1);
  ...
  if (message.command == "CAP") {
    ...
    if (message.parameters.size() > 2) {
      ...
      if (subCommand == "LS") {
        ...
        serverInfo.capabilities.merge(parseKeyValue(
            splitString(message.parameters[2 + moreCapabilitiesComing])));
...
void IRC::onPONG(const IRCMessage message) {
  ...
  lagTimer.afterSeconds([this]() { ping(); },
...
void IRC::onCTCPQuery(const IRCMessage message, const CTCPMessage ctcp) {
  if (ctcp.command == "ACTION") {
    ...
    sendCTCPResponse(message.source.nick, "SOURCE",
                     "https://github.com/BlaatSchaapCode/blaatbot2025/");
  } else if (ctcp.command == "TIME") {
#if __cpp_lib_chrono >= 201907L
    const auto zt{std::chrono::zoned_time{std::chrono::current_zone(),
                                          std::chrono::system_clock::now()}};
    sendCTCPResponse(message.source.nick, "TIME",
                     std::format("{:%FT%T%z}", zt));
#else
    // TODO: implementation for older C++ libraries (eg. Haiku)
#endif
  } else if (ctcp.command == "VERSION") {
    utils::Version version;
    sendCTCPResponse(message.source.nick, "VERSION",
                     "BlaatBot2025 " + version.m_git_commit);
...
*/
      ...
      // If digit, eat digit
      if (i + 1 < formattedString.length() && formattedString[i] >= '0' &&
          formattedString[i] <= '9')
        i++;
      // If digit, eat digit
      if (i + 1 < formattedString.length() && formattedString[i] >= '0' &&
      ...
      */
      ...
      while (pos < modestring.length()) {
        ...
        if (modeset == '+' || modeset == '-') {
          ...
          modechange_t modechange = {
              .modeset = modeset, .modechar = modechar, .parameter = -1};
          ...
          if (serverInfo.channelTypeAModes.find(modechar) !=
              std::string::npos) {
            // Mode Type A
            // Add address to list
          ...
          if (serverInfo.channelTypeBModes.find(modechar) !=
              std::string::npos) {
            // Mode Type B
          ...
          if (serverInfo.channelTypeCModes.find(modechar) !=
              std::string::npos) {
            // Mode Type C
            // Change setting on channel
          ...
          if (serverInfo.channelTypeDModes.find(modechar) !=
              std::string::npos) {
            // Mode Type D
            // Change setting on channel
            // No parameters
      ...
      for (auto &modeChange : modeChanges) {
        ...
        LOG_DEBUG("%c%c %s", modeChange.modeset, modeChange.modechar,
                  modeParameter.c_str());
...
void IRC::onTopic(const IRCMessage message) {
  ...
  if (message.parameters.size() >= 3) {
    ...
    mIRCChannels[toLower(message.parameters[1])].topicStripped =
        stripFormatting(message.parameters[2]);
  }
}

void IRC::onTopicWhoTime(const IRCMessage message) {
  if (message.parameters.size() >= 4) {
    mIRCChannels[toLower(message.parameters[1])].topicNick =
        message.parameters[2];
    mIRCChannels[toLower(message.parameters[1])].topicSetAt =
        atoi(message.parameters[3].c_str());
  }
}
  ...
  if (message.parameters.size() >= 3) {
    if (serverInfo.features.count("WHOX")) {
      ...
      send("WHO " + toLower(message.parameters[1]) +
           " %t%c%u%i%h%s%n%f%d%l%a%o%r," + std::to_string(token));
    } else {
      send("WHO " + toLower(message.parameters[1]));
    }
  }
...
void IRC::onNicknameInUse(const IRCMessage message) {
  if (!serverInfo.registrationComplete &&
      Numeric::ERR_NICKNAMEINUSE == message.command) {
...
void IRC::onData(std::vector<char> data) {
  ...
  while (true) {
    ...
    auto end = std::search(mBuffer.begin(), mBuffer.end(), eolMarker.begin(),
                           eolMarker.end());
    if (end != mBuffer.end()) {
...
void IRC::sendCTCPQuery(const std::string target, const std::string command,
...
void IRC::sendCTCPResponse(const std::string target, const std::string command,
                           const std::string parameters) {
  if (parameters.length())
